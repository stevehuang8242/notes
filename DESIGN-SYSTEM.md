...
```css
.gl{border-bottom:1px dotted var(--line);cursor:pointer}   /* ← was rgba(255,255,255,.55)；cursor 用 pointer，不要 cursor:help（hover 會出現問號游標） */
...
- **Hero**：**canonical 單一＝靠左 ticker**（`.ticker` badge + meta、細粗混排多行 h1、副標、選填主線預告 `.spine`），`padding:120px 0 40px`。不再用置中 hero。**`.hero` 不要設 `display:flex`**——靠 `.inner.wrap` 當一般 1080 wrap，左緣才會跟內文 section 對齊（flex column + `margin:0 auto` 會讓 inner 縮成內容寬並置中、與內文不齊，是 bug）。h1 強調三選一：`.hl`（`--accent` 藍，一般重點預設）/ `u`（`--pos` 綠，限真有正向語意）/ `.xline`（中性底線）。
- **Hero 主線預告 `.spine`（hero 尾段，敘事型）**：lead 標 `.sk` + 2–4 個編號 chip `.si`（內含 `.sn` 編號），**用本頁自己的話預告骨幹**（如「三大瓶頸」→ 入軌質量／太空太陽能／AI 晶片），每個 chip 兼作頁內錨點、**各自跳到不同 section**。**鐵律：spine 的職責是「預告實質」，nav 才是完整 TOC——chip 文案要帶實質、別照抄 nav 的詞，更別只挑 nav 錨點的子集（那就退回成重複導覽）。** spine 可用於任何頁（含分析型）：與 hero 下方的 Metric Strip section 並用 OK（spine 預告骨幹、strip 預告數字），**但別在 hero 內把 spine 疊在 in-hero strip 上（過載）**——若某頁的 strip 已經在 hero 裡（如 `spacex-s1`），就不另加 spine。**舊的純跳轉 `.cta`／`.btn`（兩顆只跳 `#anchor` 的按鈕）會跟 nav 重複，已退役、別再用。** podcast／影片的 YT 來源連結不放 hero（見 Footer）。
